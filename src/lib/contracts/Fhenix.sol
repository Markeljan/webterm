...
library Impl {
    function sealoutput(
        ...
        uint256 ciphertext,
        bytes32 publicKey
    ) internal pure returns (string memory reencrypted) {
        // Call the sealoutput precompile.
        reencrypted = FheOps(Precompiles.Fheos).sealOutput(
            utype,
            Common.toBytes(ciphertext),
    ...
    function cast(
        ...
        output = FheOps(Precompiles.Fheos).cast(
            utype,
            Common.toBytes(ciphertext),
            toType
        );
    ...
    function trivialEncrypt(
        ...
        output = FheOps(Precompiles.Fheos).trivialEncrypt(
            Common.toBytes(value),
            toType
        );
    ...
    function select(
        ...
        // Call the trivialEncrypt precompile.
        output = FheOps(Precompiles.Fheos).select(
            utype,
            Common.toBytes(control),
            Common.toBytes(ifTrue),
            Common.toBytes(ifFalse)
        );
...
library FHE {
    ...
    function mathHelper(
        ...
        uint256 lhs,
        uint256 rhs,
        function(uint8, bytes memory, bytes memory)
            external
            pure
            returns (bytes memory) impl
    ) internal pure returns (uint256 result) {
        bytes memory output;
    ...
    function add(euint8 lhs, euint8 rhs) internal pure returns (euint8) {
        ...
        uint256 result = mathHelper(
            Common.EUINT8_TFHE,
            unwrappedInput1,
            unwrappedInput2,
            FheOps(Precompiles.Fheos).add
        );
    ...
    function decrypt(ebool input1) internal pure returns (bool) {
        ...
        uint256 result = FheOps(Precompiles.Fheos).decrypt(
            Common.EBOOL_TFHE,
            inputAsBytes
        );
    ...
    function not(euint8 input1) internal pure returns (euint8) {
        ...
        uint256 unwrappedInput1 = euint8.unwrap(input1);
        bytes memory inputAsBytes = Common.toBytes(unwrappedInput1);
        bytes memory b = FheOps(Precompiles.Fheos).not(
            Common.EUINT8_TFHE,
            inputAsBytes
        );
...
/// @notice Performs the add operation
function operatorAddEuint128(
    euint128 lhs,
    euint128 rhs
) pure returns (euint128) {
    return FHE.add(lhs, rhs);
}
